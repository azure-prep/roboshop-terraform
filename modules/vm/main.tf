...
resource "null_resource" "ansible" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "ansible-pull -i localhost, -U https://github.com/azure-prep/roboshop-ansible.git -vvv -e app_name=${local.role_name} -e token=${var.token} -e env=${var.env} roboshop.yml"
    ]
    connection {
      type     = "ssh"
